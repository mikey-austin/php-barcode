...
class Barcode {
	...
	private static function _encode_code128($text) {
		if (preg_match('/^[0-9]+$/', $text) && (strlen($text) % 2) == 0) {
			//
			// We have an even number of digits, so encode the entire text in
			// code C.
			//
			return self::_encode_code128_variantC($text);
		} else {
			return self::_encode_code128_variantB($text);
		}
	}

	private static function _encode_code128_variantC($text) {
		$chksum = 105;
		$code_string = '';
		for ($X = 1, $i = 1; $X <= strlen($text); $X += 2, $i++) {
			$activeKey = substr($text, ($X - 1), 2);
			$code_string .= self::$code128_c_array[$activeKey];
			$chksum += (intval($activeKey) * $i);
		}

		$chksum_value = ($chksum % 103);
		if($chksum_value <= 9)
			$chksum_value = '0' . $chksum_value;
		$code_string .= self::$code128_c_array[$chksum_value];

		return '211232' . $code_string . '2331112';
	}

	private static function _encode_code128_variantB($text) {
		$chksum = 104;
